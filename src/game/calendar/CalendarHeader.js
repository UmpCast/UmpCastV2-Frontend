...
import { CustomToggle } from "../history/HistoryHeader"
...
export default function CalendarHeader(props) {
    ...
    const last_week = week_start.add(-7, "day").format("M-D-YYYY")
    const next_week = week_start.add(7, "day").format("M-D-YYYY")

    const formatted_week = week_start.format("MMM D") + " - " +
            ...
            <div className="mt-3 mb-2 mx-3 d-inline-flex w-100">
                ...
                <div className="mx-auto">
                    <div className="d-inline-flex">
                        <Button
                            as={Link}
                            to={`/calendar/${last_week}`}
                            className="p-0 bg-light border-0 mb-1 my-auto">
                            <FontAwesomeIcon className="text-primary fa-lg" icon={["fas", "chevron-left"]} />
                        </Button>
                        <h3 className="mx-3 my-auto"><strong>{formatted_week}</strong> {week_start.year()}</h3>
                        <Button
                            as={Link}
                            to={`/calendar/${next_week}`}
                            className="p-0 bg-light border-0 mb-1 my-auto">
