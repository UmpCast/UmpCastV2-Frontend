...
export default function CalendarGame(props){
    ...
    for (let i = 0; i < props.casted; i++) {
        castPreview.push(
            <ProfileIcon icon="user-alt"
                variant="light" padding="p-1" color="muted" custom={"rounded-circle mb-2 mr-2"} />
        )
    }
    ...
    for (let i = 0; i < props.roles - props.casted; i++) {
        castPreview.push(
            <ProfileIcon icon="user-alt"
                variant="light" padding="p-1" color="muted" custom="text-light border-secondary-dashed mb-2 mr-2 rounded-circle fa-sm" />
        )
    }
